...
## 단계
- **P0 (진행 중)**: repo 뼈대 + 공용 모듈 4종 + 랜딩 + 로그인 + 전적 스텁 + 게임 3종 무수정 복사 + _headers + keep-alive 복사 + CF Pages 연결.
  검증: 랜딩 브랜드/위장 토글 왕복, 게스트 로그인→닉네임 스트립, /stats/ 세션 공유·비로그인 리다이렉트(?next=), `?next=//evil.com` → `/` 폴백, JS/CSS 304 확인.
- **P1**: 005 적용(→구 프로토타입 티츄 회귀 확인 필수) + 티츄를 공용 모듈로 이관(net.js/auth.js/스크린auth/크롬/토글/버전체크 삭제, chrome.js+chrome.css 공용화) + /stats/ 구현(4탭+랭킹 5판↑+최근 20건). 검증: 새 도메인 4탭 풀 매치, 전적 반영, GoTrue 중복 경고 없음.
- **P2**: 006 + /baseball/ 재작성(2인 대기실, TURN/Metered 크리덴셜 전량 제거). 검증: 체크섬 패리티, 콘솔에서 bb_secrets select 0행, 상대턴 guess 서버 거부, 이어하기, 재대국 선공 규칙, 3선승→전적.
- **P3**: 007 + /omok/ 재작성. 검증: tests.html 전건, SQL 패리티 전건, 삼삼 거부/사삼 허용/장목 거부/5우선/백 육목 승, 이어하기, 흑백 교대, 전적.
